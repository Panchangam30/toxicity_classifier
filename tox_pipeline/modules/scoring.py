...
def aggregate_scores(module_outputs):
    ...
    """
    # Extract values (use 0 if missing)
    general_tox = module_outputs.get("general_tox", {}).get("carcinogenicity", 0)
    ...
    immunotox = module_outputs.get("immunotox", {}).get("immunotoxicity", 0)
    ...
    structural_alert_penalty = 1 if alerts.get("alert_count", 0) > 0 else 0
